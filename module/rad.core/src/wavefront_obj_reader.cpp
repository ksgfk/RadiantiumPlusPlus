...
WavefrontObjReader::WavefrontObjReader(Unique<std::istream> stream) {
...
WavefrontObjReader::WavefrontObjReader(const std::filesystem::path& file) {
  ...
  if (!std::filesystem::exists(file)) {
    throw RadFileNotFoundException("cannot find file: {}", file.string());
  }
}
...
TriangleModel WavefrontObjReader::ToModel(const std::string& name) const {
  ...
  throw RadArgumentException("no model name: {}", name);
}
...
TriangleModel WavefrontObjReader::ToModel(const std::vector<WavefrontObjFace>& faces) const {
  ...
  if (p.size() >= std::numeric_limits<UInt32>::max()) {
    throw RadOutOfRangeException("model tooo big, vertex size {}", p.size());
  }
  if (ind.size() >= std::numeric_limits<UInt32>::max()) {
    throw RadOutOfRangeException("model tooo big, index size {}", ind.size());
  }
