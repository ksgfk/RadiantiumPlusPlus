...
class RAD_EXPORT_API Light {
 public:
   ...
   * @param xi2 采样光源上的点
   * @param xi3 采样发出光线的方向
   * @return std::tuple<Ray, Spectrum, PositionSampleResult, Float, Float> [光线, 光线携带能量, 光源上的点采样结果, 光源上的点采样pdf, 光线方向采样pdf]
   */
  virtual std::tuple<Ray, Spectrum, PositionSampleResult, Float, Float> SampleLe(
      const Vector2& xi2,
      const Vector2& xi3) const = 0;
  /**
   * @brief 光源出发的光线向某个方向前进的概率密度
   *
   * @param psr 光源上的点采样结果
   * @param dir 光线出发方向
   * @return std::pair<Float, Float>
   */
  virtual std::pair<Float, Float> PdfLe(const PositionSampleResult& psr, const Vector3& dir) const = 0;
