...
class Path final : public SampleRenderer {
 public:
  Path(BuildContext* ctx, Unique<Scene> scene, const ConfigNode& cfg) : SampleRenderer(ctx, std::move(scene), cfg) {
...
class PathFactory final : public RendererFactory {
 public:
  PathFactory() : RendererFactory("path") {}
  ~PathFactory() noexcept override = default;
  Unique<Renderer> Create(BuildContext* ctx, Unique<Scene> scene, const ConfigNode& cfg) const override {
    return std::make_unique<Path>(ctx, std::move(scene), cfg);
  }
};

Unique<RendererFactory> _FactoryCreatePathFunc_() {
  return std::make_unique<PathFactory>();
}
