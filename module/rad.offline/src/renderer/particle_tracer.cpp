...
/**
 * @brief 光粒子追踪，从光源出发，再尝试与相机连接，评估重要性
 */
class ParticleTracer final : public Renderer {
 public:
  ParticleTracer(BuildContext* ctx, Unique<Scene> scene, const ConfigNode& cfg) : Renderer(ctx, std::move(scene), cfg) {
...
class PTracerFactory final : public RendererFactory {
 public:
  PTracerFactory() : RendererFactory("particle_tracer") {}
  ~PTracerFactory() noexcept override = default;
  Unique<Renderer> Create(BuildContext* ctx, Unique<Scene> scene, const ConfigNode& cfg) const override {
    return std::make_unique<ParticleTracer>(ctx, std::move(scene), cfg);
  }
};

Unique<RendererFactory> _FactoryCreatePTracerFunc_() {
  return std::make_unique<PTracerFactory>();
}
