...
class BDPT final : public Renderer {
 public:
  ...
  BDPT(BuildContext* ctx, Unique<Scene> scene, const ConfigNode& cfg) : Renderer(ctx, std::move(scene), cfg) {
...
class BdptFactory final : public RendererFactory {
 public:
  BdptFactory() : RendererFactory("bdpt") {}
  ~BdptFactory() noexcept override = default;
  Unique<Renderer> Create(BuildContext* ctx, Unique<Scene> scene, const ConfigNode& cfg) const override {
    return std::make_unique<BDPT>(ctx, std::move(scene), cfg);
  }
};

Unique<RendererFactory> _FactoryCreateBdptFunc_() {
  return std::make_unique<BdptFactory>();
}
