...
class Skybox final : public Light {
 public:
  Skybox(BuildContext* ctx, const Matrix4& toWorld, const ConfigNode& cfg) {
    _toWorld = Transform(toWorld);
    _flag = (UInt32)LightType::Infinite;
    _map = ConfigNodeReadTexture(ctx, cfg, "radiance", Color24f(1));
...
class SkyboxFactory final : public LightFactory {
 public:
  SkyboxFactory() : LightFactory("skybox") {}
  ~SkyboxFactory() noexcept override = default;
  Unique<Light> Create(BuildContext* ctx, const Matrix4& toWorld, const ConfigNode& cfg) const override {
    return std::make_unique<Skybox>(ctx, toWorld, cfg);
  }
};

Unique<LightFactory> _FactoryCreateSkyboxFunc_() {
  return std::make_unique<SkyboxFactory>();
}
