...
class Disney final : public Bsdf {
 public:
  ...
  Disney(BuildContext* ctx, const ConfigNode& cfg) {
    _baseColor = ConfigNodeReadTexture(ctx, cfg, "base_color", Color24f(0.5f));
    _roughness = ConfigNodeReadTexture(ctx, cfg, "roughness", Float32(0.5));
    _hasAnisotropic = ConfigHasValue("anisotropic", cfg);
    _anisotropic = ConfigNodeReadTexture(ctx, cfg, "anisotropic", Float32(0));
    _hasSpecTrans = ConfigHasValue("spec_trans", cfg);
    _specTrans = ConfigNodeReadTexture(ctx, cfg, "spec_trans", Float32(0));
    _hasSheen = ConfigHasValue("sheen", cfg);
    _sheen = ConfigNodeReadTexture(ctx, cfg, "sheen", Float32(0));
    _hasSheenTint = ConfigHasValue("sheen_tint", cfg);
    _sheen_tint = ConfigNodeReadTexture(ctx, cfg, "sheen_tint", Float32(0));
    _hasFlatness = ConfigHasValue("flatness", cfg);
    _flatness = ConfigNodeReadTexture(ctx, cfg, "flatness", Float32(0));
    _hasSpecTint = ConfigHasValue("spec_tint", cfg);
    _specTint = ConfigNodeReadTexture(ctx, cfg, "spec_tint", Float32(0));
    _hasMetallic = ConfigHasValue("metallic", cfg);
    _metallic = ConfigNodeReadTexture(ctx, cfg, "metallic", Float32(0));
    _hasClearcoat = ConfigHasValue("clearcoat", cfg);
    _clearcoat = ConfigNodeReadTexture(ctx, cfg, "clearcoat", Float32(0));
    _clearcoatGloss = ConfigNodeReadTexture(ctx, cfg, "clearcoat_gloss", Float32(0));
 ...
 private:
  static bool ConfigHasValue(const std::string& name, const ConfigNode& cfg) {
    if (cfg.HasNode(name)) {
      auto&& node = cfg.GetData()->operator[](name);
      if (node.is_number()) {
  ...
  Unique<TextureMono> _roughness;
  Unique<TextureMono> _anisotropic;
  Unique<TextureMono> _sheen;
  Unique<TextureMono> _sheen_tint;
  Unique<TextureMono> _specTrans;
  Unique<TextureMono> _flatness;
  Unique<TextureMono> _specTint;
  Unique<TextureMono> _clearcoat;
  Unique<TextureMono> _clearcoatGloss;
  Unique<TextureMono> _metallic;
  Float _eta;
  Float _specular;
...
class DisneyFactory final : public BsdfFactory {
 public:
  DisneyFactory() : BsdfFactory("disney") {}
  ~DisneyFactory() noexcept override = default;
  Unique<Bsdf> Create(BuildContext* ctx, const ConfigNode& cfg) const override {
    return std::make_unique<Disney>(ctx, cfg);
  }
};

Unique<BsdfFactory> _FactoryCreateDisneyFunc_() {
  return std::make_unique<DisneyFactory>();
}
