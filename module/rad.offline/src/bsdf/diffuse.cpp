...
class Diffuse final : public Bsdf {
 public:
  Diffuse(BuildContext* ctx, const ConfigNode& cfg) {
    ...
    _reflectance = ConfigNodeReadTexture(ctx, cfg, "reflectance", Color24f(0.5f));
  }
...
class DiffuseFactory final : public BsdfFactory {
 public:
  DiffuseFactory() : BsdfFactory("diffuse") {}
  ~DiffuseFactory() noexcept override = default;
  Unique<Bsdf> Create(BuildContext* ctx, const ConfigNode& cfg) const override {
    return std::make_unique<Diffuse>(ctx, cfg);
  }
};
