...
class Sphere final : public Shape {
 public:
  Sphere(BuildContext* ctx, const Matrix4& toWorld, const ConfigNode& cfg) {
    ...
    Matrix3 rotation = ConfigNodeReadOrDefaultRotate(cfg, "rotate_uv", Matrix3::Identity());
    ...
    _giveEmbreeData = (EmbreeSphere*)AlignedMalloc(16, sizeof(EmbreeSphere));
  }
...
class SphereFactory final : public ShapeFactory {
 public:
  SphereFactory() : ShapeFactory("sphere") {}
  ~SphereFactory() noexcept override = default;
  Unique<Shape> Create(BuildContext* ctx, const Matrix4& toWorld, const ConfigNode& cfg) const override {
    return std::make_unique<Sphere>(ctx, toWorld, cfg);
  }
};
